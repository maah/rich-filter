...
class Filter {
     ...
     */
    addWord(word, wordsToWhitelist) {
        word = word.toLowerCase();
        if (this.blacklist.indexOf(word) == -1) {
            this.blacklist.push(word);
            if (!wordsToWhitelist)
                this.whitelist.push('');
            else if (typeof wordsToWhitelist === 'string')
                this.whitelist.push(wordsToWhitelist);
            else
                this.whitelist.push(wordsToWhitelist.join(','));
        }
        // Update whitelist instead
        else if (wordsToWhitelist) {
            const index = this.blacklist.indexOf(word);
            if (typeof wordsToWhitelist === 'string')
                this.whitelist[index] = wordsToWhitelist;
            else
                this.whitelist[index] = wordsToWhitelist.join(',');
        }
        return this;
    }
    /**
     ...
     * @returns The updated filter
     */
    editWord(word, wordsToWhitelist) {
        word = word.toLowerCase();
        if (this.blacklist.indexOf(word) != -1) {
            const index = this.blacklist.indexOf(word);
            if (typeof wordsToWhitelist === 'string')
     ...
     * @returns findAll: A list of objects
     */
    findWithIndexStrict(message, findAll) {
        const split = message.split(' ');
        let allWords = [];
        message = message.toLowerCase();
        for (let i = 0; i < this.blacklist.length; i++) {
     ...
     */
    censor(message, findAll, replaceChar) {
     ...
     * @returns The message with filtered words censored or removed
     */
    censorStrict(message, findAll, replaceChar) {
        let words = this.findWithIndexStrict(message.toLowerCase(), findAll);
        return this.doCensor(words, message, findAll, replaceChar);
    }
    doCensor(words, message, findAll, replaceChar) {
        let censored = "";
